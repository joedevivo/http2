...
-spec is_padded(http2_frame:header()) -> boolean().
...
-spec read_possibly_padded_payload(binary(), http2_frame:header())
                                  -> binary().
...
-spec read_padded_payload(binary(), http2_frame:header())
                         -> binary().
...
-spec read_unpadded_payload(binary(), http2_frame:header())
                           -> binary().
